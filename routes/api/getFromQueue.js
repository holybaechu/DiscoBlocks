...
router.get('/', async function(req, res) {
    ...
    if (ipChecker(req.headers['x-forwarded-for'] || req.socket.remoteAddress) != true){console.log('error: ' + usernameToGet); res.status(400).json({success: false, errors: [{message: "This ip is not seems to be allowed to access this api."}]}); return;}

    if (globalsModule.verifyQueue[usernameToGet]) {
        res.json({ success: true, data: {tag: globalsModule.verifyQueue[usernameToGet].tag} })
    }else{
        console.log('error: ' + usernameToGet)
